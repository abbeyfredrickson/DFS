...
void TaintGraphTraversal::readSrcSnkFromFile(const string &filename)
{
    ...
    while (getline(file, line))
    {
        ...
        string apiSet = line.substr(start + 2, end - start - 2);

        stringstream ss(apiSet);
        string api;
        // read each word
        while (ss >> api)
        {
            lineNum == 1 ? checker_source_api.insert(api) : checker_sink_api.insert(api);
        }
        lineNum++;
    }
...
bool TaintGraphTraversal::aliasCheck(const CallICFGNode *src, const CallICFGNode *snk)
{
    ...
    const SVFVar *vsrc = srcRetNode->getActualRet();
    ...
    return ander->alias(ander->getPts(vsrc->getId()), ander->getPts(vsnk->getId()));
}
